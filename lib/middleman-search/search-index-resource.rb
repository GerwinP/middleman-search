  ...
  module Sitemap
    class SearchIndexResource < ::Middleman::Sitemap::Resource
      ...
      def build_index
        # Build js context
        libs = []
        libs << lunr_resource('elasticlunr.min.js')
        ...
        source << 'elasticlunr.Index.prototype.indexJson = function () {return JSON.stringify(this.toJSON());};'

        # Build lunr based on config
        source << 'middlemanSearchIndex = elasticlunr(function () {'

        # Do not store the original document
        source << 'this.saveDocument(false);'

        # Use language if set
        source << "this.use(elasticlunr.#{@language});" if @language != 'en'

        # Use autogenerated id field as reference
        source << "this.setRef('id');"

        # Add functions to pipeline (just registering them isn't enough)
        @pipeline.each do |name, function|
          source << "var #{name} = #{function};"
          source << "elasticlunr.Pipeline.registerFunction(#{name}, '#{name}');"
          source << "this.pipeline.add(#{name});"
        end

        # Define fields with boost
        @fields.each do |field, opts|
          next if opts[:index_field] == false

          source << "this.addField('#{field}');"
        end
        ...
        @app.sitemap.resources.each_with_index do |resource, id|
          begin
              ...
              next if resource.data['index_document'] == false
              ...
              @callback&.call(to_index, to_store, resource)

              source << "middlemanSearchIndex.addDoc(#{to_index.merge(id: id).to_json});"
        ...
        json = context.eval('middlemanSearchIndex.indexJson()')
