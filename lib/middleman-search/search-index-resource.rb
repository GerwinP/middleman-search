    ...
    class SearchIndexResource < ::Middleman::Sitemap::Resource
      ...
      def build_index
        ...
        @pipeline.each do |name, function|
          source << "elasticlunr.Pipeline.registerFunction((#{function}), '#{name}');"
        end

        # Build lunr based on config
        source << "elasticlunr.middlemanSearchIndex = elasticlunr(function () {"
        ...
        @pipeline.each do |name, function|
          source << "this.pipeline.add(elasticlunr.Pipeline.getRegisteredFunction(#{name}));"
        end
        ...
        source << "ealsticlunr.Pipeline.load(this.pipeline);"
        ...
        @app.sitemap.resources.each_with_index do |resource, id|
          begin
              ...
              source << "elasticlunr.middlemanSearchIndex.add(#{to_index.merge(id: id).to_json});"
        ...
        json = context.eval('elasticlunr.middlemanSearchIndex.indexJson()')
