...
ElasticLunrJS-based search for Middleman.

For more details about elasticlunr, see their [http://elasticlunr.com/](documentation).
...
activate :search do |search|
  ...
  search.fields = {
    ...
    content: {boost: 50},
    url:     {index_field: false, store: true},
    author:  {boost: 30}
  }
end
```
...
- `index_field` Whether to index this field, defaults to true
...
### Query-time boosting
elasticlunr supports query-time boosting instead of setting up boosting weights in the index building procedure:
```javascript
var max_search_entries = 50;
...
### Expanded tokens
To increase the recall of the search results, one can expand the query tokens. This means that when a user searches `micro`, and both `microscope` and `microwave` are in the index, both will be returned:
```javascript
var max_search_entries = 50;

var result = []; //initialize empty array

lunrIndex.search(request.term, {
  expand: true
}).forEach( function (item, index) {
  if ( index < max_search_entries ) {
    result.push(lunrData.docs[item.ref]);
...
- `index` contains the serialised lunr.js index, which you can load via `elasticlunr.Index.load(lunrData.index)`
- `docs` is a map from the autogenerated document ids to an object that contains the attributes configured for storage

You will typically load the `index` into a elasticlunr index instance, and then use the `docs` map to look up the returned value and present it to the user.

You should also `require` the `elasticlunr.min.js` file in your main sprockets javascript file (if using the asset pipeline) to be able to actually load the index:

```javascript
//= require elasticlunr.min
```

If you're using elasticlunr's i18n capabilities, you should also load the Stemmer support and language files (in that order) here:
...
$.ajax({
  ...
  success: function(data) {
    lunrData = data;
    lunrIndex = elasticlunr.Index.load(lunrData.index);
  }
});
```
